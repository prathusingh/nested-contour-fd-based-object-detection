...
class ImageInfoCV : public UMat{
public:
	ImageInfoCV(int r, int l, int t) :UMat(r, l, t) {};
	std::vector<ImageInfoCV	*> GradientDericheX(std::vector< ImageInfoCV *>, double alphaDerive, double alphaMoyenne);
	std::vector<ImageInfoCV	*> GradientDericheY(std::vector< ImageInfoCV *>, double alphaDerive, double alphaMoyenne);
};
...
std::vector<ImageInfoCV	*> ImageInfoCV::GradientDericheX(std::vector< ImageInfoCV *>, double alphaDerive, double alphaMoyenne)//double alphaDerive,double alphaMoyenne)
{
...
std::vector<ImageInfoCV	*> ImageInfoCV::GradientDericheY(std::vector< ImageInfoCV *>, double alphaDerive, double alphaMoyenne)
{
	std::vector<ImageInfoCV	*> r;
	/*
...
int main(int argc, char** argv)
{
	...
	ImageInfoCV mm(m.rows, m.cols, m.type());
	m.copyTo(mm);
	imshow("original", mm);
	vector<ImageInfoCV*> v;
	v = mm.GradientDericheX(v, 1, 1);
	double minVal, maxVal;
	minMaxLoc(*v[0], &minVal, &maxVal);
	v[0]->convertTo(m, CV_8U, 255 / (maxVal - minVal), -255 * minVal / (maxVal - minVal));
	imshow("Gx", m);
	waitKey();
	return 0;
};
